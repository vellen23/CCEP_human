function [] = cut_block_edf_LT(EEG_block, stim_list,type,block_num, Fs, subj,BP_label, path_pp)
    ...
    save(sprintf('%s/data_blocks/%s_BP_%s_%02d/scalpEEG.mat',path_pp,subj, type, block_num),'scalpEEG','scalpFs','labels','-v7.3');
    if ~isempty(inECG)
        EMG    = EEG_block(inECG,:);
        EMG_label = labels_all(inECG);
        save(sprintf('%s/data_blocks/%s_BP_%s_%02d/EMG.mat',path_pp,subj, type, block_num),'EMG','EMG_label', 'Fs','-v7.3');
    end
       
    s = dir(sprintf('%s/data_blocks/%s_BP_%s_%02d/%s_BP_%s_%02d.mat',path_pp,subj, type, block_num,subj, type, block_num));
    size_MB = max(vertcat(s.bytes))/1e6;
    create_metadata(subj,sprintf('%s/data_blocks/%s_BP_%s_%02d/%s_BP_%s_%02d.mat',path_pp,subj, type, block_num,subj, type, block_num), Fs, nch, dp, size_MB,start,stop)
