...
def compute_subj(subj, metric='LL'):
    ...
    con_trial_Ph.insert(0, 'zLL', con_trial_Ph.groupby(['Stim', 'Chan'])['LL_BL'].transform(
        lambda x: (x - x.mean()) / x.std()).values)
    con_trial_Ph = con_trial_Ph[(con_trial_Ph.zLL < 6)&(con_trial_Ph.zLL > -6)].reset_index(drop=True)
    con_trial_Ph = con_trial_Ph.drop(columns='zLL')
    # remove outliers based on high LL
    con_trial_Ph.insert(0, 'zLL', con_trial_Ph.groupby(['Stim', 'Chan', 'Int'])['LL_BL'].transform(
        lambda x: (x - x.mean()) / x.std()).values)
    con_trial_Ph = con_trial_Ph[(con_trial_Ph.zLL < 6) & (con_trial_Ph.zLL > -6)].reset_index(drop=True)
    con_trial_Ph = con_trial_Ph.drop(columns='zLL')
        ...
        if os.path.isfile(V_path)*skip:
            con_trial_nan = con_trial_Ph[
                (con_trial_Ph.Stim == sc) & (con_trial_Ph.d > -1) & (
                            con_trial_Ph.Artefact < 1)].reset_index(drop=True)  # con_trial_Ph.copy(deep=True)
            ...
            if np.sum(np.isnan(con_trial_nan[metric])) > 0: con_trial_nan[metric] = \
                con_trial_nan.groupby(['Chan', 'SleepState', 'Int'])[metric].transform(
            ...
            if np.sum(np.isnan(con_trial_nan[metric])) > 0: con_trial_nan[metric] = \
                con_trial_nan.groupby(['Chan', 'Int'])[metric].transform(
                    lambda x: x.fillna(x.mean()))
            # create an input matrix with rows (Chan) and columns (Num)
            NMF_input = np.zeros((len(labels_all), len(np.unique(con_trial_nan.Num).astype('int'))))
            i = 0
            nums = con_trial_nan.Num.values # trial numbers
            nums, idx = np.unique(nums, return_index=True)
            # fill input matrix with values based on Chan and Num
            for num in nums.astype('int'):
                ...
                i = i + 1
            # todo: fill zeros with the mean of the row
            NMF_input = np.fillzeros(NMF_input ... )
...
print('START')
metrics = ['LL']  # 'sN2','sN1',
for subj in ["EL028"]:  # ["EL016", "EL011", "EL004", "EL005", "EL010",  "EL015", "El014"]:  # ["EL011","EL015", "EL010",  "EL012", "El014"]: #, "EL004", "EL010", "EL011", "EL012", "El014"]:  # "EL012", "EL013",
    for m in metrics:
        compute_subj(subj, m)
#         try:
