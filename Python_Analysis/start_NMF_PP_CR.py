...
def compute_subj(subj, metric='LL'):
    ...
    # adding extra num_ix to have a seperate one for the fake SP
    con_trial_Ph.insert(0, 'Num_ix', con_trial_Ph.Num)
    con_trial_Ph.loc[con_trial_Ph.Int == 0, 'Num_ix'] = con_trial_Ph.loc[con_trial_Ph.Int == 0, 'Num_ix'] + 0.5
    con_trial_Ph.Num_ix = (con_trial_Ph.Num_ix * 2).astype('int')
     # remove bad channels
    ...
    con_trial_Ph.loc[
        con_trial_Ph.Artefact != 0, metric] = np.nan  # con_trial_Ph.loc[con_trial_Ph.Artefact!=0, metric] =np.nan
    # remove outliers
    ...
    for i_sc, sc in zip(np.arange(len(stimchans)), stimchans):
        ...
        if run_again*os.path.isfile(V_path):
            ...
            if np.sum(np.isnan(con_trial_nan[metric])) > 0: con_trial_nan[metric] = \
                con_trial_nan.groupby(['Stim', 'Chan', 'Sleep', 'Int', 'IPI'])[metric].transform(
                    lambda x: x.fillna(x.mean()))
            if np.sum(np.isnan(con_trial_nan[metric])) > 0: con_trial_nan[metric] = \
                con_trial_nan.groupby(['Stim', 'Chan', 'Int', 'IPI'])[metric].transform(
                    lambda x: x.fillna(x.mean()))
            if np.sum(np.isnan(con_trial_nan[metric])) > 0: con_trial_nan[metric] = \
                con_trial_nan.groupby(['Stim', 'Chan', 'IPI'])[metric].transform(
                    lambda x: x.fillna(x.mean()))

            NMF_input = np.zeros((len(labels_all), len(np.unique(con_trial_nan.Num_ix).astype('int'))))
            i = 0
            nums = con_trial_nan.Num_ix.values
            nums, idx = np.unique(nums, return_index=True)

            for num in nums.astype('int'):
                # for num in nums[np.sort(idx)].astype('int'):
                dat = con_trial_nan[con_trial_nan.Num_ix == num]
        ...
        for rk in [ranks[ix0]]:  # rank_sel:  # range(k0, k1 + 1):
            ...
            # columns label
            col0 = ['Stim', 'Int', 'IPI', 'Block', 'Hour', 'Date', 'Sleep']  #  'Hour',
            col = ['Stim', 'Int', 'IPI', 'Block', 'Hour', 'Date', 'Sleep']  #  'Hour',
            W_col = []
            H_col = []
            ...
            summ = con_trial_nan.groupby(['Num_ix'], as_index=False)[col0].mean()
            ...
            NMFf.plot_H_trial(con_nmf, 'IPI', 'Int', title_LL, nmf_fig_path)
            ...
            if np.isin(rk, rank_sel):
                ...
                file = nmf_path + 'PP_CR_' + str(p) + 'rk' + str(rk) + '.csv'
                con_nmf.to_csv(file, index=False, header=True)
                # NMFf.plot_H_trial_IPI(pd_con_nnmf, 'Int', 'Block', title_LL, nmf_fig_path)
                NNMF_ass = NMFf.get_NMF_Stim_association_PP(con_nmf, H_col)
                ...
                for H in np.unique(NNMF_ass.loc[NNMF_ass.Stim == sc, 'H']):
                    NMFf.plot_H_IPI_cond(con_nmf, H, 'SleepState', nmf_fig_path)
                #          file = nmf_fig_path + 'IO_CR_AUC_LLpeak_rk' + str(rk) + '_H' + str(H) + '_Stim_' + labels_all[
                #             sc]  # +'.npy'
                #         title = subj + ' -- ' + labels_all[sc] + ', H' + str(H) + '/' + str(rk)
                #         NMFf.plot_NMF_AUC_Ph(pd_con_nnmf, sc, H, title, file)
                p = p + 1
    print(subj + ' ---- DONE ------ ')
...
print('START')
metrics = ['LL']  # 'sN2','sN1',
for subj in ["EL011", "EL004", "EL005", "EL010", "EL016", "EL015",
             "El014"]:  # ["EL011","EL015", "EL010",  "EL012", "El014"]: #, "EL004", "EL010", "EL011", "EL012", "El014"]:  # "EL012", "EL013",
    for m in metrics:
        compute_subj(subj, m)