...
class main:
    def __init__(self, subj, protocol_type):
        ...
        df = pd.read_excel(os.path.join(self.path_patient + "/infos/", file_name),
                           sheet_name='Par_circ')  # ChanP  ChanN  Int [mA]  IPI [ms]  ISI [s]  Num_P
        ...
        for i in range(len(stim_chan)):
            self.StimChans.append(self.labels_all[(np.array(data.chan_BP_P.values) == stim_chan[i, 0]) & (
                        np.array(data.chan_BP_N.values) == stim_chan[i, 1])][0])
    ...
    def cut_blocks_stim(self, exp, block_num):
        ...
        for u in range(len(self.StimChans)):
            ...
            for o in range(block_num):
                ...
                EEG_block[:, :, :] = np.NaN
                matfile = h5py.File(
                    self.path_patient + '/Data/experiment' + str(
                        exp) + '/data_blocks/time/' + self.subj + "_BP_CR" + str(
                        h) + "_pp/" + self.subj + "_BP_CR" + str(h) + "_pp.mat", 'r')['EEGpp']
                EEGpp = matfile[()].T
                for s in range(len(stim_table)):
                    trig = stim_table.TTL_DS.values[s]
                    if np.int(trig + self.dur[0, 1] * self.Fs) > EEGpp.shape[1]:
                        EEG_block[:, s, 0:EEGpp.shape[1] - np.int(trig + self.dur[0, 0] * self.Fs)] = EEGpp[:, np.int(
                            trig + self.dur[0, 0] * self.Fs):EEGpp.shape[1]]
                        ...
                        EEG_block[:, s, :] = EEGpp[:, np.int(trig + self.dur[0, 0] * self.Fs):np.int(
                            trig + self.dur[0, 1] * self.Fs)]
                if o == 0:
            ...
            stim_list.to_csv(
                self.path_patient + '/Analysis/Circadian_PP/data/Resps_CR_' + StimChan + '_day' + str(exp) + '.csv',
                index=False, header=True)
            np.save(self.path_patient + '/Analysis/Circadian_PP/data/Resps_CR_' + StimChan + '_day' + str(exp) + '.npy',
                    EEG_stims)
    ...
    def cut_blocks_h(self, exp, block_num):
        for o in range(block_num):
            ...
            EEG_block[:, :, :] = np.NaN
            matfile = h5py.File(
                self.path_patient + '/experiment' + str(exp) + '/data_blocks/time/' + self.subj + "_BP_CR" + str(
                    h) + "_pp.mat",
                'r')['EEGpp']
            ...
            for s in range(len(stim_table)):
                ...
                if np.int(trig + dur[0, 1] * Fs) > EEGpp.shape[1]:
                    EEG_block[:, s, 0:EEGpp.shape[1] - np.int(trig + dur[0, 0] * self.Fs)] = EEGpp[:, np.int(
                        trig + self.dur[0, 0] * self.Fs):EEGpp.shape[1]]
                elif np.int(trig + self.dur[0, 0] * self.Fs) < 0:
                    EEG_block[:, s, abs(np.int(trig + self.dur[0, 0] * self.Fs)):] = EEGpp[:, 0:np.int(
                        trig + self.dur[0, 1] * self.Fs)]
            ...
            np.save(self.path_patient + '/experiment' + str(exp) + '/Circadian_PP/All_resps_CR' + str(h) + '.npy',
                    EEG_block)
    ...
    def get_LL_c(self, exps, w, sc, c):
        ...
        for u in range(len(exps)):
            ...
            stim_table = pd.read_csv(
                self.path_patient + '/Analysis/Circadian_PP/data/Resps_CR_' + StimChan + '_day' + str(
            ...
            stim_list_spef = stim_table[(stim_table.ChanP == ChanP) & (stim_table.noise == 0) & (((
                                                                                                          stim_table.Int_cond == 2) & (
                                                                                                              stim_table.IPI_ms > 120)) | (
                                                                                                             stim_table.IPI_ms == 0))]
            stimNum = stim_list_spef.Number.values
        ...
        # saving
        LL_all.to_csv(self.path_patient + '/Analysis/Circadian_PP/data/LL/LL_' + StimChan + '-' + str(
            self.labels_all[c]) + '.csv',
    ...
    def plot_stimNum_IPI_Int(self, sc, c, exp, IPI, Int):
        ...
        stim_list_spef = stim_table[(stim_table.noise == 0) & (stim_table.IPI_ms == IPI) & (
                    stim_table.Int_prob == Int)]  # &(stim_table.noise==0)
        stimNum = stim_list_spef.Number.values
        ...
        plt.title(
            str(self.subj + ', experiment: ' + str(exp) + ", Stim: " + StimChan + ', Resp: ' + self.labels_all[c]))
    ...
    def plot_LL_IPI(self, sc, c):
        ...
        print(self.labels_all[c])
        LL_all = pd.read_csv(
            self.path_patient + '/Analysis/Circadian_PP/data/LL/LL_' + StimChan + '-' + str(
                self.labels_all[c]) + '.csv')
