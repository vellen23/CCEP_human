...
class main:
    def __init__(self,subj,path_patient):
                ...
                for s in range(len(stim_table)):
                    ...
                    if np.int64(trig + self.dur[0, 1] * self.Fs) > EEGpp.shape[1]:
                        EEG_block[:, s, 0:EEGpp.shape[1] - np.int64(trig + self.dur[0, 0] * self.Fs)] = EEGpp[:,
                                                                                            np.int64(trig + self.dur[0, 0] * self.Fs):
                                                                                            EEGpp.shape[1]]
            ...
            if len(stim_table)>0:
                ...
                for s in range(len(stim_table)):
                    ...
                    SO_block[:, :, s, :] = SOP[:, :, np.int64(trig + self.dur[0, 0] * self.Fs):np.int64(
                        trig + self.dur[0, 1] * self.Fs)]
                np.save(path_patient + '/Analysis/'+folder+'/data/HGP_resps_' + str(block) + '_' + condition + '.npy',
                        HG_block)
                np.save(
    ...
    def concat_osc(self, type):
        ...
        for p in range(len(files)):
            ...
            HGP_block  = np.load(self.path_patient + '/Analysis/'+folder+'/data/HGP_resps_' + str(block) + '_' + cond + '.npy')
            SOP_block = np.load(
    ...
    def cut_BM(self, path, block):
        ...
        np.save(path_patient + '/Analysis/BrainMapping/data/All_resps_' + str(block) + '_' + condition + '.npy',
                EEG_block)
        stim_table.to_csv(
            path_patient + '/Analysis/BrainMapping/data/Stim_list_' + str(block) + '_' + condition + '.csv', index=False,
            header=True)  # scat_plot
    def cut_BM_HGP(self, path, block):
        ...
        if len(stim_table)>0:
            ...
            for s in range(len(stim_table)):
                trig        = stim_table.TTL_DS.values[s]
                HG_block[:, :, s, :] = HGP[:, :, np.int(trig + self.dur[0, 0] * self.Fs):np.int(trig + self.dur[0, 1] * self.Fs)]
    ...
    def concat_BM(self, EEGup=False):
        ...
        for p in prots:
            ...
            conds[p - 1] = cond
            if EEGup:
                EEG_block = np.load(self.path_patient + '/Analysis/BrainMapping/data/All_resps_' + str(p) + '_' + cond + '.npy')
