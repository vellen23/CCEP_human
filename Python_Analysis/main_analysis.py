...
def run_first(subj, prots=['BM', 'IO', 'PP'], cut=1, con_trial=1, hyp=0, run_concat=1):
    ...
    if run_concat:
        start_cut_resp.compute_list_update(subj, prots=['BM', 'IO'])
        for p in prots:  # , 'InputOutput'
            if p == 'BM':
                ...
                f = 'PairedPulse'
            concat.concat_resp_condition(subj, folder=f, cond_folder='CR', EEG = 0, skip=0)

    ###### BM Analysis
    ### get GT

    run_sig_con.start_subj_GT(subj, folder='BrainMapping', cond_folder='CR', cluster_method='similarity', skipt_GT=0, skip_surr=1, trial_sig_labeling=1)


def run_update(subj):
    BM_blocks.update_peaks(subj, cond_folder='CR')
    ...
    # run_sig_con.trial_significance(subj, folder='BrainMapping', cond_folder='CR', fdr=False, p=0.05)
...
for subj in subjs:  # ''El009', 'EL010', 'EL011', 'EL012', 'EL013', 'EL015', 'EL014','EL016', 'EL017'"EL021", "EL010", "EL011", "EL012", 'EL013', 'EL014', "EL015", "EL016",
    if thread:
        ...
        _thread.start_new_thread(run_update, (subj,))
    else:
        # run_update(subj)
        run_first(subj, prots=['BM', 'IO'], cut=0, con_trial=0, hyp=0, run_concat=0)
        run_update(subj)
print('DONE ALL')
if thread:
    while 1:
