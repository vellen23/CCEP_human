...
def convert(edfFile, outputData):
    edfData = None
    try:
    ...
    if edfData is not None:
        dateAndTime = edfData.info['meas_date']
        outputData = outputData[:-3]+'_'+dateAndTime.strftime('%Y%m%d')+'_'+dateAndTime.strftime('%H%M%S')+'.h5'
        write_new_hdf5(outputData)
        write_edf_data(outputData, edfData)


def convert_patient(subj):
...
if __name__ == "__main__":
    subjs = ["EL011", "EL012", "EL020", "EL021",
             "EL022", "EL024", "EL026", "EL027","EL018", "EL009", "EL017", "EL025"]

    for subj in subjs:
        t0 = datetime.now()
        convert_patient(subj)
